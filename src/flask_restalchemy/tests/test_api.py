...
def test_post(client):
    ...
    emp3 = db.session.get(Employee, 3)
...
def test_put(client):
    ...
    assert resp.status_code == HTTPStatus.OK
    emp3 = db.session.get(Employee, 1)
...
def test_init_app(flask_app, db_session, client):
    api = Api()
    api.init_app(flask_app)
    api.add_model(Company, url="/company_lazy", view_name="company_lazy")

    resp = client.post("/company_lazy", data={"name": "lazy corp"})
    assert resp.status_code == HTTPStatus.CREATED
    company_id = resp.get_json()["id"]

    resp = client.get(f"/company_lazy/{company_id}")
    assert resp.status_code == HTTPStatus.OK
    assert resp.get_json()["name"] == "lazy corp"


def test_register_column_serializer():
    class MySerializer(ColumnSerializer):
        def load(self, value):
            return value
    ...
    original = ModelSerializer.EXTRA_SERIALIZERS[:]
    try:
        Api.register_column_serializer(MySerializer, predicate)
        assert ModelSerializer.EXTRA_SERIALIZERS[-1] == (MySerializer, predicate)
    finally:
        ModelSerializer.EXTRA_SERIALIZERS[:] = original

    with pytest.raises(TypeError, match="Invalid serializer class"):
        Api.register_column_serializer(object, predicate)
...
def test_http_verbs(flask_app, client, methods):
    ...
    api = Api(flask_app)
    # Pass a copy: register_view removes items from the list in-place, which would
    # corrupt the `methods` variable used in the assertions below.
    api.add_resource(
    ...
    def expect(resp, verb):
        if verb in methods:
            assert resp.status_code == HTTPStatus.OK
        else:
            # Blocked verbs return 405 when the URL exists for another method,
            # or 404 when no rule at all is registered for that URL pattern.
            assert resp.status_code != HTTPStatus.OK

    expect(client.get("/ping"), "GET_COLLECTION")
    expect(client.get("/ping/1"), "GET")
    expect(client.post("/ping"), "POST")
    expect(client.delete("/ping/1"), "DELETE")
    expect(client.put("/ping/1"), "PUT")
