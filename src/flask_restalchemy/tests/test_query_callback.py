...
def sample_relation_query(parent_query, model):
    ...
    :return:
    """
    subquery = select(employee_department.c.employee_id).where(
        employee_department.c.department_id == 100
    )
    if parent_query:
        query = parent_query.filter(model.id.in_(subquery))
    else:
        query = db.session.query(model).filter(model.id.in_(subquery))
    return query
...
def sample_model_query(parent_query, model):
    ...
    :return:
    """
    all_heroes_subquery = select(employee_department.c.employee_id).where(
        employee_department.c.department_id == 200
    )
    all_companies_heroes_only_subquery = select(Employee.company_id).where(
        Employee.id.in_(all_heroes_subquery)
    )
    if parent_query:
...
def sample_property_query(parent_query, model):
    ...
    if parent_query:
        ...
        query = db.session.query(model).filter(model.id != self_id)
    return query
