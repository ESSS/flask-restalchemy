...
from flask_restalchemy.tests.sample_model import Employee, Company, Department, Address, employee_department, db
...
def create_test_sample(db_session):
    ...
    raynor = Employee(id=11, firstname='Jim', lastname='Raynor', company=terrans, departments=[basic, heroes])
    marine = Employee(id=12, firstname='John', lastname='Doe', company=terrans, departments=[basic])
    kerrigan = Employee(id=13, firstname='Sarah', lastname='Kerrigan', company=terrans, departments=[heroes])

    tassadar = Employee(id=21, firstname='Tassadar', lastname='The Warrior', company=protoss, departments=[heroes])
    zealot = Employee(id=22, firstname='Aiur', lastname='Zealot', company=protoss, departments=[basic])

    zergling = Employee(id=31, firstname='Rush', lastname='Lings', company=zerg, departments=[basic])
...
def sample_relation_query(model, parent_query=None):
    ...
    :return:
    '''
    subquery = db.session.query(employee_department.c.employee_id).filter(employee_department.c.department_id == 100).subquery()
    if parent_query:
